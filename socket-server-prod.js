...
server.listen(PORT, () => {
    ...
    const baseUrl = process.env.APP_URL || process.env.LARAVEL_URL || `${protocol}://${host}`;
    
    console.log(`🚀 Socket.IO server running on port ${PORT}`);
    console.log(`🌍 Environment: ${NODE_ENV}`);
    console.log(`🔗 Health check: ${baseUrl}:${PORT}/health`);
    console.log(`🔗 Health check (local): http://localhost:${PORT}/health`);
    console.log(`📊 Status: ${baseUrl}:${PORT}/status`);
    console.log(`📊 Status (local): http://localhost:${PORT}/status`);
});
